...
function getQueueNumber() {
  ...
  // Kirim permintaan nomor antrian dengan keluhan
...
ipcRenderer.on("queue-result", (event, data) => {
  ...
  const queueParts = data.number.split("-");
  const abbreviatedNumber = `${queueParts[0][0]}-${
    queueParts[queueParts.length - 1]
  }`;
  document.getElementById("queueNumber").textContent = abbreviatedNumber;
